...
public class ProductBigDataQuestions {
    ...
    public static void main(String[] args) {
        ...
        int numberChemicalsInAllProducts = productList.stream().map(Product::getNumberOfChemicals).reduce(0, (c1, c2) -> c1 + c2);
        ...
        long numberProductsFourOrMore = productList.stream().filter(p -> p.getNumberOfChemicals() >= 4).count();
        ...
        String companyMostProducts = companyProductMap.keySet().stream().max((c1, c2) -> Integer.compare(companyProductMap.get(c1).size(), companyProductMap.get(c2).size())).get();
       ...
       // Hint: Use a nested stream (one stream of productList and then a separate stream for each list of each product).
        productList.stream().forEach(product -> product.getChemicals().stream().forEach(chemicalName -> chemicalProductMap.get(chemicalName).add(product)));
       ...
       String mostOccurringChemical = chemicalProductMap.keySet().stream().max((chem1, chem2) -> Integer.compare(chemicalProductMap.get(chem1).size(), chemicalProductMap.get(chem2).size())).get();
