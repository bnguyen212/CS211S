...
public class CustomerInput extends Application {
    ...
    private void parseFile(File file) {
    	...
    	try (Scanner scan = new Scanner(file)) {
    		while (scan.hasNext()) {
    			...
    			int numOfOrders = Integer.parseInt(lineScan.next());
    			lineScan.close();
    			...
    			Customer cust = new Customer(id, numOfOrders);
    			customerList.add(cust);
    		}
    		statusText.setText("Added " + customerList.size() + " customers!");
    		statusText.setFill(Color.GREEN);
    		resultText.setText("Total: " + getTotalOrderCount(customerList) + " orders");
    		resultText.setVisible(true);
    		uploadButton.setDisable(true);
    	} catch (FileNotFoundException | InvalidCharacterException | NumberFormatException e) {
    ...
    private static int getTotalOrderCount(ArrayList<Customer> list) {
    	int totalCount = 0;
    	for (Customer c : list) {
    		totalCount += c.getNumberOfOrders();
    	}
    	return totalCount;
    }
